...
const SolutionSection = () => {
  ...
  return <section id="solution" className="relative overflow-hidden px-8 py-32 md:px-16"><div className="absolute inset-0 gradient-lunar pointer-events-none" /><div className="relative mx-auto max-w-6xl"><motion.div
    ref={titleRef}
  ...
  /><p className="mx-auto mt-8 max-w-2xl font-body text-sm leading-relaxed text-muted-foreground">
    ...
    },
    {
      icon: Shield,
      title: "DETERMINISTIC",
      desc: "Two functions \u2014 quoteExactIn never reverts, swapExactIn fills or reverts with a reason. Every fill produces a machine-readable receipt: the oracle mid, the spread charged, the execution mode. Hard onchain constraints: deadline, minOut, oracle freshness, capacity limits."
    }
  ].map((item, i) => {
    ...
    ><div className="mb-5 flex h-10 w-10 items-center justify-center rounded-sm border border-primary/20 bg-primary/5 transition-colors duration-500 group-hover:border-primary/40 group-hover:bg-primary/10"><item.icon className="h-5 w-5 text-primary/70 transition-colors duration-500 group-hover:text-primary" /></div><h3 className="mb-3 font-display text-sm tracking-[0.2em] text-foreground">{item.title}</h3><p className="font-body text-sm leading-relaxed text-muted-foreground">{item.desc}</p></motion.div>;
  })}</div>{
    /* Execution Flow */
  }<motion.div
    ref={flowRef}
  ...
  ><h3 className="mb-10 text-center font-display text-sm tracking-[0.3em] text-muted-foreground">
            EXECUTION FLOW
          </h3><div className="flex flex-col items-center gap-4 md:flex-row md:justify-center md:gap-0">{flowSteps.map((step, i) => <div key={step.label} className="flex items-center gap-4"><motion.div
    initial={{ opacity: 0, scale: 0.9 }}
    animate={flowInView ? { opacity: 1, scale: 1 } : {}}
    transition={{ duration: 0.5, delay: 0.3 + i * 0.2 }}
    className="card-lunar rounded-sm px-6 py-4 text-center w-[200px]"
  ><div className="font-mono text-xs text-primary">{step.label}</div><div className="mt-1 font-body text-[11px] text-muted-foreground">{step.sub}</div></motion.div>{i < flowSteps.length - 1 && <motion.div
    initial={{ opacity: 0 }}
    animate={flowInView ? { opacity: 1 } : {}}
    transition={{ duration: 0.3, delay: 0.5 + i * 0.2 }}
  ><ArrowRight className="h-4 w-4 text-primary/40 hidden md:block" /></motion.div>}</div>)}</div></motion.div>{
    /* Execution Modes */
  }<motion.div
    ref={modesRef}
    initial={{ opacity: 0, y: 30 }}
  ...
  ].map((mode, i) => <motion.div
    key={mode.num}
    initial={{ opacity: 0, y: 20 }}
    animate={modesInView ? { opacity: 1, y: 0 } : {}}
    transition={{ duration: 0.6, delay: i * 0.15 }}
    className="card-lunar flex items-start gap-4 rounded-sm p-6"
  ><span className="font-display text-2xl text-primary/30">{mode.num}</span><div><h4 className="font-display text-xs tracking-[0.2em] text-foreground">{mode.title}</h4><p className="mt-1 font-body text-sm text-muted-foreground">{mode.desc}</p></div></motion.div>)}</motion.div></div></section>;
};
var stdin_default = SolutionSection;
export {
