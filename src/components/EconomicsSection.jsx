...
const EconomicsSection = () => {
  ...
  return <section id="economics" className="relative px-8 py-32 md:px-16"><div className="mx-auto max-w-6xl"><motion.div
    ref={titleRef}
  ...
  /><p className="mx-auto mt-8 max-w-2xl font-body text-sm leading-relaxed text-muted-foreground">
            ...
            Revenue is transactional, not subsidy-driven.{" "}<span className="text-foreground">No token emissions. No liquidity mining.</span></p></motion.div><div className="grid gap-16 lg:grid-cols-2">{
    /* Fee Structure */
  }<motion.div
    ref={feeRef}
  ...
  ><h3 className="mb-8 font-display text-sm tracking-[0.3em] text-muted-foreground">
              FEE STRUCTURE
            </h3><div className="space-y-1">{feeComponents.map((fee, i) => <motion.div
    key={fee.name}
  ...
  ><div className="mt-0.5 h-1.5 w-1.5 rounded-full bg-primary/50 shrink-0" /><div><span className="font-body text-sm text-foreground">{fee.name}</span><p className="mt-0.5 font-body text-xs text-muted-foreground">{fee.desc}</p></div></motion.div>)}<p className="pt-3 font-body text-xs text-muted-foreground italic">
  ...
  ><h3 className="mb-8 font-display text-sm tracking-[0.3em] text-muted-foreground">
              VALUE FLOW
            </h3><div className="mb-6 card-lunar rounded-sm px-5 py-4 text-center"><span className="font-body text-xs tracking-wider text-muted-foreground">SWAP VOLUME</span><span className="mx-3 text-primary/40">→</span><span className="font-body text-xs tracking-wider text-foreground">SPREAD REVENUE</span></div><div className="space-y-3">{valueFlows.map((flow, i) => <motion.div
    key={flow.num}
    initial={{ opacity: 0, y: 20 }}
    animate={flowInView ? { opacity: 1, y: 0 } : {}}
    transition={{ duration: 0.5, delay: 0.2 + i * 0.12 }}
    className="card-lunar flex items-start gap-4 rounded-sm p-5"
  ><span className="font-display text-lg text-primary/30">{flow.num}</span><div><h4 className="font-display text-xs tracking-[0.2em] text-foreground">{flow.title}</h4><p className="mt-1 font-body text-sm text-muted-foreground">{flow.desc}</p></div></motion.div>)}</div><div className="mt-6 rounded-sm border border-border/50 bg-secondary/20 px-5 py-4"><p className="font-body text-xs text-muted-foreground"><span className="text-foreground">Partner Economics:</span> Routing integrators earn
                configurable rebates on every fill, paid in the output token from realized spread revenue.
              </p></div></motion.div></div></div></section>;
};
var stdin_default = EconomicsSection;
export {
