...
const SecurityRoadmapSection = () => {
  ...
  return <>{
    /* Security */
  }<section className="relative overflow-hidden px-8 py-32 md:px-16"><div className="absolute inset-0 gradient-lunar pointer-events-none" /><div className="relative mx-auto max-w-6xl"><motion.div
    ref={secRef}
  ...
  /></motion.div><div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{securityFeatures.map((feat, i) => {
    ...
    ><div className="mx-auto mb-4 flex h-10 w-10 items-center justify-center rounded-sm border border-primary/20 bg-primary/5 transition-colors duration-500 group-hover:border-primary/40"><feat.icon className="h-5 w-5 text-primary/70" /></div><h3 className="mb-2 font-display text-[11px] tracking-[0.2em] text-foreground">{feat.title}</h3><p className="font-body text-xs leading-relaxed text-muted-foreground">{feat.desc}</p></motion.div>;
  })}</div></div></section>{
    /* Roadmap */
  }<section id="roadmap" className="relative px-8 py-32 md:px-16"><div className="mx-auto max-w-4xl"><motion.div
    ref={roadRef}
    initial={{ opacity: 0, y: 30 }}
  ...
  /><div className="space-y-8">{phases.map((phase, i) => {
    const ref = useRef(null);
    ...
    ><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-sm border border-border bg-secondary/50"><span className="font-display text-sm text-primary">{phase.phase}</span></div><div className="pt-1"><h3 className="font-display text-xs tracking-[0.2em] text-foreground">{phase.title}</h3><p className="mt-1 font-body text-sm text-muted-foreground">{phase.desc}</p></div></motion.div>;
  })}</div></div></div></section></>;
};
var stdin_default = SecurityRoadmapSection;
export {
